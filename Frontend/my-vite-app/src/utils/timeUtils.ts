...
/**
 * Intelligens időszámítás - a státusz alapján dönt, hogy melyik dátumig számoljon
 * @param startDate - Az aukció kezdési időpontja (opcionális)
 * @param expiredDate - Az aukció befejezési időpontja
 * @returns Időinformáció és hogy melyik eseményig számol
 */
export const calculateSmartTimeLeft = (
  ...
  if (status === 'UPCOMING' && startDate) {
    // UPCOMING: startDate-ig számolunk vissza
    targetDate = parseBackendDate(startDate);
    targetEvent = 'start';
  } else {
    // ACTIVE vagy CLOSED: expiredDate-ig számolunk vissza
